...
// code form macaron.v1/static.go
// Copyright 2013 Martini Authors
// Copyright 2014 The Macaron Authors
// Copyright 2016 Kirill Zhuharev
//
...
var (
	AllowedSuffixes = []string{".js", ".css", ".woff", ".jpg", ".png", ".jpeg"}
)
...
func prepareStaticOption(opt StaticOptions) StaticOptions {
	// Defaults
	...
	if opt.FileSystem == nil {
		opt.FileSystem = newStaticFileSystem("")
	}
	return opt
}
...
func prepareStaticOptions(options []StaticOptions) StaticOptions {
	...
	return prepareStaticOption(opt)
}
...
func staticHandler(ctx *macaron.Context, log *log.Logger, opt StaticOptions) bool {
	...
	hasSuff := false
	for _, suff := range AllowedSuffixes {
		if strings.HasSuffix(strings.ToLower(file), suff) {
			hasSuff = true
		}
	}
	...
	targetPath := filepath.Join("sites/", ctx.Req.Host, "public", file)
	targetPath2 := filepath.Join("public", file)

	f, err := opt.FileSystem.Open(targetPath)
	if err != nil {
		f, err = opt.FileSystem.Open(targetPath2)
		if err != nil {
			color.Red("%s", err)
			return false
		}
...
func Static(staticOpt ...StaticOptions) macaron.Handler {
	opt := prepareStaticOptions(staticOpt)

	return func(ctx *macaron.Context, log *log.Logger) {
		log.Println("aim static")
...
func Statics(opt StaticOptions, dirs ...string) macaron.Handler {
	...
	for i := range dirs {
		opts[i] = prepareStaticOption(opt)
	}
